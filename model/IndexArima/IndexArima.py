...
path = r'/data/st01/trace_data/IndexArima/resources'
for filename in os.listdir(path):

    data = pd.read_excel('./resources/'+filename, index_col = u'日期',header = 0)
    print("正在预测："+filename)
    # print(data.head())
    ...
    #预测未来12个单位
    predictions=loaded.forecast(11)
    #预测结果为：
    ...
    for i in range(12):
        if(i+1) < 10:
            dateStr = '2015-0'+str(i+1)+'-01'
        else:
            dateStr = '2015-' + str(i + 1) + '-01'
        date.append(dateStr)
    for i in range(12):
        if(i+1) < 10:
            dateStr = '2016-0'+str(i+1)+'-01'
        else:
            dateStr = '2016-' + str(i + 1) + '-01'
        date.append(dateStr)
    for i in range(12):
        if(i+1) < 10:
            dateStr = '2017-0'+str(i+1)+'-01'
        else:
            dateStr = '2017-' + str(i + 1) + '-01'
        date.append(dateStr)
    for i in range(12):
        if(i+1) < 10:
    ...
    dict = {'key': filename.split("_")[0], 'date': date, 'index': sale}
