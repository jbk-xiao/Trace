...
fh = logging.FileHandler('BERT-NER.log')
fh.setLevel(tf.logging.DEBUG)
...
flags.DEFINE_string("vocab_file", '../bert/chinese_L-12_H-768_A-12/vocab.txt',
                    "The vocabulary file that the BERT model was trained on.训练BERT模型的词汇表文件。")
    ...
    def _read_data(cls, input_file):
        ...
        with open(input_file,encoding='utf-8') as f:
            ...
            for line in f:
                ...
                if len(contends) == 0:
                    l = ' '.join([label for label in labels if len(label) > 0])   #获得这一句话里所有的标签
                    w = ' '.join([word for word in words if len(word) > 0])    #获得这一句话里所有的词
                    lines.append([l, w])
                    words = []
                    labels = []
                    continue
...
def write_tokens(tokens,mode):    #测试的时候使用
    if mode=="test":
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
    ...
    def model_fn(features, labels, mode, params):
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            hook_dict[
                'global_steps'] = tf.compat.v1.train.get_or_create_global_step()  # tf.train.get_or_create_global_step()
